...
public:
	...
	int& operator*()			// Оператор разыменования возвращает значение по адресу типа Int.
	{							// Если этот метод будет применен (вызовем) для константного объекта, он просто не вызовется,
		return Temp->Data;		// поэтому нужно два метода: один из них должен быть константным, он будет возвращать константную ссылку,
	}							// второй не константный и будет возвращать не константную ссылку.
};
